...
| Tests / CI | ✅ **452 passed** python + 119 forge + 95 node, 6-job CI green |
...
- **Python: 452 tests** (incl. anvil-gated on-chain integration, skipped when anvil is down — CI boots a node so they genuinely run) — core, estimator, instrument, oracle_client, sim, tape, services (x402-circle, marketplace, webhooks, terminal-bundle, keeper, desk), top-level `tests/`.
