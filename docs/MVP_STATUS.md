...
| Tests / CI | ✅ **661 passed** python + 156 forge + 136 node, 6-job CI green |
...
- **Python: 661 tests** (incl. anvil-gated on-chain integration, skipped when anvil is down — CI boots a node so they genuinely run) — core, estimator, instrument, oracle_client, sim, tape, services (x402-circle, marketplace, webhooks, terminal-bundle, keeper, desk), top-level `tests/`.
...
- **Node: 157 tests** (136 terminal + 21 agent) + `tsc` type-checks.
