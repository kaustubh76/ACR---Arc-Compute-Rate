...
| Tests / CI | ✅ **695 passed** python + 156 forge + 165 node, 6-job CI green |
...
- **Python: 695 tests** (incl. anvil-gated on-chain integration, skipped when anvil is down — CI boots a node so they genuinely run) — core, estimator, instrument, oracle_client, sim, tape, services (x402-circle, marketplace, webhooks, terminal-bundle, keeper, desk), top-level `tests/`.
