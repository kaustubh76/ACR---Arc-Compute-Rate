...
- **State:** A well-structured, **honestly-documented** system. Three big sessions are done: (1) a core-infra **hardening** pass that made the docs' claims actually true, (2) a **real Circle/Arc live-wiring** pass, and (3) a **Circle Agent Stack completion** pass — all five pillars (Nanopayments, Agent Wallets, Agent Marketplace, Circle CLI, Circle Skills) now exist in the repo, with the buyer side implemented against the *verified* Gateway API and buyer-SDK shapes. Everything runs offline on the simulator today; the live Circle/Arc paths exist and are tested but need the user's credentials to run against real infra.
- **Tests green:** **165 Python tests** (163 pass + 2 anvil-gated skips), **32 Foundry tests**, **10 node tests** (`apps/agent`), `ruff` + glossary checks clean, terminal + agent `tsc`/`next build` clean. Hermetic — `make test` needs no credentials and no Circle SDK.
- **Deliberately deferred:** ERC-4337 Paymaster (Arc USDC *is* the gas token → gasless is intrinsic), Next.js 15 upgrade, and a couple of Circle-doc specifics isolated behind config (see **Landmines**). Git: the estimator core, contracts, seller API, buyer agent, deploy kit, and test suites are committed and pushed to `github.com/kaustubh76/ACR---Arc-Compute-Rate`; `apps/terminal`, `docs/`, `.github/`, and the architecture `.excalidraw` canvases are still untracked as the incremental commit finishes.
...
| Contracts | **Real** (EIP-712 verify, staleness, pause, 2-step; registry nonce/deadline) | `contracts/src/*.sol` | 32 forge tests incl. invariants. |
...
| API service | **Real** (lifespan refresh, poster loop, rolling-window store, locks) | `services/index_api/*` | Prints evolve; `vol` real; poster posts when a signer + oracle are configured. |
| Terminal | **Real** (SSR + honest snapshot) | `apps/terminal/` | Next 14.2.x pinned. |
...
- **Python: 165 tests** (163 pass + 2 skipped — the anvil round-trip in `test_onchain.py`). Breakdown: core 14, estimator 24, instrument 5, oracle_client 21, sim 6, tape 16, services 67 (incl. x402-circle + marketplace + webhooks + terminal-bundle), `tests/` 12.
- **Foundry: 32 tests** (`ACROracle` 17, `AttestationRegistry` 10, invariants 5 with `fail_on_revert=true`).
- **Node: 10 tests** (`apps/agent`, node:test with stubbed fetch — DevPayer two-act flow, rejection, price parsing, catalog filters, spend-cap stop + price-over-cap refusal) + `tsc` type-checks for agent and terminal.
...
- **The repo `.env` holds broken placeholder values.** It was copied from an early `.env.example` draft whose inline `# comments` after `=` get parsed as the *value* (e.g. `circle_wallet_id = "# the oracle..."`). Tests are protected by `conftest.py`, but **a real live run needs a clean `.env`** — re-copy from the current (fixed) `.env.example` or fill real values. Do not trust the current `.env`.
...
- **Still flagged (confirm at first live run):** the Circle SDK method names (`_RealCircleClient` / `_RealCircleDeployer` in `signer.py` / `deploy_circle.py`); the real Arc event signature + `size`/`service` derivation (`ArcSource`); Circle CLI verb/flag spellings (`circle <resource> --help` — the CLI is v0.0.x and moving); one real Gateway round trip has not yet been run (needs user credentials).
- **Git: incremental commit in progress.** The estimator core, contracts, seller API, buyer agent, cloud deploy kit, and test suites are committed and pushed to `github.com/kaustubh76/ACR---Arc-Compute-Rate`. Still untracked as the drip-commit finishes: `apps/terminal`, `docs/`, `.github/` (CI), and the architecture `.excalidraw` canvases.
...
3. `make api` → `curl -i /prints` returns 402 + b64 `PAYMENT-REQUIRED`; pay via a Circle Gateway buyer wallet → 200 + `PAYMENT-RESPONSE` txHash. `ACR_TAPE_SOURCE=arc` serves the live tape; the poster loop Circle-signs each print.

---

## Deferred / next-steps backlog

- **One real Arc-testnet round trip** (`docs/agent-runbook.md`): wallet + faucet + Gateway deposit → `make agent-live` → receipts on /exchange. All shapes are pre-verified; this exercises credentials + funding only.
- Submit ACR to Circle's Agent Marketplace directory (manual form at agents.circle.com/services — materials listed in the runbook §6).
...
- First git commit (nothing is committed yet).
...
- `~/.claude/plans/check-and-rethink-that-modular-music.md` — the working plan file (Circle/Arc phase detail + this doc's plan).
