...
- Python: **691 passed** (incl. 22 anvil-gated on-chain tests — CI starts a node so they genuinely run; they skip only on a machine without anvil), spanning core, estimator, instrument, oracle_client, sim, tape, services (incl. x402-circle + marketplace + webhooks + terminal-bundle), and `tests/`.
...
- **Node: 159 terminal tests** (`apps/terminal`, node:test — a dedicated CI job) + **31** in the `apps/agent` suite (node:test with stubbed fetch — DevPayer two-act flow, rejection, price parsing, catalog filters, spend-cap stop + price-over-cap refusal, the agent card, the reroute decision through the whole loop) + `tsc` type-checks for agent and terminal.
