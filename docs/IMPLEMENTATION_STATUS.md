...
- Python: **693 passed** (incl. 22 anvil-gated on-chain tests — CI starts a node so they genuinely run; they skip only on a machine without anvil), spanning core, estimator, instrument, oracle_client, sim, tape, services (incl. x402-circle + marketplace + webhooks + terminal-bundle), and `tests/`.
