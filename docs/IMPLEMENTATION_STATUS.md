...
- **Python: 356 tests** (incl. 9 anvil-gated on-chain tests — CI starts a node so they run; they skip only on a machine without anvil) (incl. anvil-gated on-chain integration (skipped without anvil) — the anvil round-trip in `test_onchain.py`), spanning core, estimator, instrument, oracle_client, sim, tape, services (incl. x402-circle + marketplace + webhooks + terminal-bundle), and `tests/`.
