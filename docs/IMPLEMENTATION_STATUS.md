...
- **Node: 156 terminal tests** (`apps/terminal`, node:test — a dedicated CI job) + **31** in the `apps/agent` suite (node:test with stubbed fetch — DevPayer two-act flow, rejection, price parsing, catalog filters, spend-cap stop + price-over-cap refusal, the agent card, the reroute decision through the whole loop) + `tsc` type-checks for agent and terminal.
