...
- **Tests green:** **355 Python tests** (incl. anvil-gated on-chain integration), **60 Foundry tests** (incl. the ACRFutures suite + invariants), **93 node tests** (`apps/terminal`, a dedicated CI job) + the `apps/agent` node suite, `ruff` + glossary checks clean, terminal + agent `tsc`/`next build` clean. Hermetic — `make test` needs no credentials and no Circle SDK.
