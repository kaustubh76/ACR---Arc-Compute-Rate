...
- **Node: 142 terminal tests** (`apps/terminal`, node:test — a dedicated CI job) + **13** in the `apps/agent` suite (node:test with stubbed fetch — DevPayer two-act flow, rejection, price parsing, catalog filters, spend-cap stop + price-over-cap refusal) + `tsc` type-checks for agent and terminal.
