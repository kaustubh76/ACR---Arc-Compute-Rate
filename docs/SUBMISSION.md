...
| Python suite | `pytest packages services tests` | ✅ **566 passed** — including 20 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
...
| Terminal | `npm test && next build` | ✅ **123/123 node tests** (14 suites: buy plan · connection ladder · oracle codec · futures codec · edition · glossary · plain-edition coverage · desk phase · futures book · read result · chain constants · price formatting · indexed tape · verified humans) + clean build (10 pages + 25 API routes) |
