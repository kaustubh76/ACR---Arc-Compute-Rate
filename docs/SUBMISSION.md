...
| Python suite | `pytest packages services tests` | ✅ **452 passed** — including 9 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
| Resistance gate | `scripts/eval.py --hours 12 --check` | ✅ all 4 checks PASS |
| Contracts | `forge test -vvv` | ✅ **119 passed** (17 oracle + 26 oracle-v2 + 10 registry + 16 futures + 10 feed-access attestor + 25 receipt mirror + 15 invariants, `fail_on_revert=true`) |
