...
| Python suite | `pytest packages services tests` | ✅ **669 passed** — including 22 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
...
| MCP plugin | `cd mcp && npx tsc --noEmit && npm test` | ✅ tsc clean, **14/14** (re-measured **2026-09-13**) — the tool contract: an operation name is forwarded, caller query text never is, and with `ACR_AGENT_PRIVATE_KEY` set every tool call carries an agent card |
| Buyer agent | `npm run build && npm test` | ✅ tsc clean, **10/10** |
| Terminal | `npm test && next build` | ✅ **142/142 node tests** (16 suites: buy plan · connection ladder · oracle codec · futures codec · edition · glossary · plain-edition coverage · desk phase · futures book · read result · chain constants · price formatting · indexed tape · verified humans · the agent gate · the agent card) + clean build (10 pages + 26 API routes) |
