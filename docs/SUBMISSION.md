...
| Python suite | `pytest packages services tests` | ✅ **555 passed** — including 20 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
...
| Terminal | `npm test && next build` | ✅ **106/106 node tests** (12 suites: buy plan · connection ladder · oracle codec · futures codec · edition · glossary · plain-edition coverage · desk phase · futures book · read result · chain constants · price formatting) + clean build (9 pages + 18 API routes) |
