...
4. **Close the loop.** The autonomous hedger does both legs: **the print it purchases is the input to the position it takes, and the log says so.** It pays $0.0001 for the rate, reads its own book, sizes the gap to its mandate, and trades the difference — from a Circle agent wallet, with no human in the loop.

That fourth rung is the product in one sentence. Every other machine here does one leg: the CI buyer pays for data it never acts on; the heartbeat trades without paying for what it trades on. The hedger is the one that makes an economic decision, and its decision log names the receipt it bought before the fill it took.
...
**The circularity, named.** Both sides of the futures book are ours, and both x402 payers are ours. Growing the receipt tape from 11 to 31 bought twenty more proofs that the rail works and not one more customer. What is real is the mechanism: real USDC, real signatures, real settlement, and an agent whose logged decisions can be checked against the chain. What is not yet real is demand.
...
| Python suite | `pytest packages services tests` | ✅ **364 passed** — including 9 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
