...
|---|---|---|---|---|
| **The press** | Developer-Controlled custody `0x8366968f…` | Arc gas for **every** write the system makes: six prints an hour across two oracle generations, two transactions per mirrored receipt, the keeper's heartbeat. The `0.41/day` estimate is the three-print era; on 2026-09-12 it ran ~3/day while the tape was refilled and reached **0.006 USDC** before a hand refuel — so it is now watched with a **hard 1.0 USDC floor** on `/ops` and in `verify_live`, not a warning | **Nothing — deliberately.** It publishes what the estimator computed; a benchmark whose publisher exercises judgement is a quote | `PricePosted` on ACROracle; `/ops` carries the cadence **and the press balance** |
...
**The circularity, named.** Both sides of the futures book are ours, and all three x402 payers are ours (the third is a demo human's wallet). Growing the receipt tape from 11 to 42 bought more proofs that the rail works and not one more customer. What is real is the mechanism: real USDC, real signatures, real settlement, and an agent whose two legs are separately checkable by anyone — its payments as Gateway batch references on the public receipts tape, its position and fills as Arc transactions. What is not yet real is demand.
...
- **AttestationRegistry** [`0x23ae3E1A306824F0CBA0b6561cB7E5502f63dFb7`](https://testnet.arcscan.app/address/0x23ae3E1A306824F0CBA0b6561cB7E5502f63dFb7) — 6 real seller attestations (the hedonic-quality flywheel).
...
- **ACROracleV2** [`0xFCa038CEad7b9e9aa8fDAfc9e80253835fB8FFEA`](https://testnet.arcscan.app/address/0xFCa038CEad7b9e9aa8fDAfc9e80253835fB8FFEA) — **a print that says how it was made.** v1 publishes a number; v2 publishes the number *and* the four fields that let anyone re-derive it: `policyHash` (a digest of the cleaning parameters and algorithm version, so "the keeper decides what is wash" becomes a checkable claim rather than a request for trust), `windowStart`/`windowEnd` (what the print actually summarizes), and `humanAdjustedBound` (`0` means *not computed* — never defaulted to the wallet bound, which would claim humans are as cheap to buy as wallets). The six v1 signed fields are a strict **prefix** of v2's ten, so no existing test vector shifts and one signing path serves both generations. Deployed at block 60,541,786 and **backfilled with v1's last 48 h** — 131 prints, strictly monotone per index, level with v1 and never ahead of it. v1 stays the venue's feed because `ACRFutures`' oracle pointer is immutable; the press dual-posts, and a v2 failure is recorded and stepped over so a tape problem can never stop the press.
- **ReceiptMirror** [`0xA9CD5b9503aeA88EB343333E842D2860b263DB65`](https://testnet.arcscan.app/address/0xA9CD5b9503aeA88EB343333E842D2860b263DB65) — **an off-chain Gateway settlement, made indexable.** `FeedAccessAttestor` answers *did this wallet pay?*; this answers *what did it buy, from whom, at what price* — the tape a benchmark is computed from. Two-phase and EIP-712-signed by the same Circle poster wallet: `openSettlement` anchors payer, seller, index and amount at the settled timestamp, `finalizeSettlement` adds the service and quantity once known. Four guards keep the tape honest rather than merely full — a settlement cannot be dated in the future, cannot be older than `MAX_MIRROR_LAG` (1 h), cannot go backwards for a payer, and a late one is admissible only through an `onlyOwner` path that **flags it `late`** instead of silently backdating it. Deployed at block 60,541,749.

- **HumanIdMirror** [`0x7f41faA38F35F1FABfc76Df5B1618fC8d0c0d8e5`](https://testnet.arcscan.app/address/0x7f41faA38F35F1FABfc76Df5B1618fC8d0c0d8e5) — **who is one person, without ever saying who.** The tape knows wallets; a manipulation bound denominated in wallets is a bound denominated in something free. This records, per rotation window, which payer wallets belong to one human — as a cluster id `keccak(nullifier, salt, window)`, **never a World ID nullifier** (a public chain has no delete, and a durable nullifier against every wallet in a fleet would be a permanent doxx). The salt is committed on chain and `/humanid/info` reports `salt_matches_commitment: true`, because a wrong salt derives ids that match nothing and every surface would read "no humans" while every transaction succeeded. Ids **rotate every 7 days**, so a resolution expires rather than lingers; the Terminal's People chip says "out of date" when it has, instead of showing zero. **Live, with money behind it**: two demo humans, four wallets, resolved for window 2958; on 2026-09-12 those wallets settled 48 real purchases across all six sellers and **44 are on the tape stamped `human: true`**, so `/rating` now reports `human_share` ≈ 0.22 where it read 0.0 and `distinctHumans` (2) visibly differs from `distinctPayers` (4) — the only thing that makes grouping by human show on screen. **Honest framing:** these are Sandbox identities, flagged `sandbox` all the way onto the chain and into every count that includes them.
- **The agent gate, and the screen in front of it** — *an agent-to-agent call has to say who is calling, and what it sends has to be inspected.* An agent presents an EIP-712 **card** in an `AGENT-CARD` header; the gate recovers the signature and believes the key, not a registry — nobody is enrolled and there is no allowlist to run. The domain deliberately names **no `verifyingContract`**, which is what permissionless means here, so the card carries its own `audience` and a **15-minute bound** in place of one. Three tiers, and only the third is scarce: *anonymous* gets the host ceiling (behind one proxy, a global one); *carded* gets a per-key bucket, and keys are free; *human* gets a per-**person** bucket because the card's cluster claim is checked against `HumanIdMirror.clusterOf` on chain — ten keys belonging to one human share one budget, and a claim the chain cannot confirm is a **401, not a downgrade**. A verified identity is excused the shared host ceiling, which is what makes the tiers real rather than a second constraint. In front of the carded tape read sits **Google Cloud Model Armor**, both directions, fail-closed, live from `asia-south1` — on 2026-09-12 it refused a prompt injection **by name (`pi_and_jailbreak`) over the public internet**, driven through `POST /graph/query` rather than by calling the module. `/agent/whoami` reports the tier a caller reached; `/armor/info` reports **which backend answered**, because the offline pattern floor and Model Armor look identical from outside and only one of them is a screen. `scripts/demo_agent.py` runs all ten acts — the three tiers, the sybil answer, a stranger's cluster, last week's id, a wrong audience, a 30-day TTL, the injection, the counters — against the deployed host with one exit code.
...
**Three distinct payers** have settled real USDC through Circle Gateway against this gate — the count that matters, because revenue from a single wallet we control proves plumbing rather than demand. The durable, in-repo artifact is [`services/index_api/index_api/receipts_live.jsonl`](../services/index_api/index_api/receipts_live.jsonl): **42 Gateway-settled receipts** (re-measured 2026-09-13 against the live `/marketplace/receipts`; `scripts/archive_receipts.py` pulls them and the redeploy refuses to run while production is ahead of the archive), scheme `exact`, `eip155:5042002`, $0.0001 for index routes and per-seller for the fleet, deduped by `tx_ref`. The five newest carry the **tier the buyer's agent card earned** — two `carded`, three `human` — the first settlements on the tape that name the person behind the wallet. It lives under `services/` rather than `data/` because that directory is in both `.gitignore` and `.dockerignore`, so a file there reaches neither the repo nor the image — an earlier version of this claim was false for exactly that reason.
...
| Python suite | `pytest packages services tests` | ✅ **677 passed** — including 22 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
...
| MCP plugin | `cd mcp && npx tsc --noEmit && npm test` | ✅ tsc clean, **16/16** (re-measured **2026-09-13**) — the tool contract: an operation name is forwarded, caller query text never is, with `ACR_AGENT_PRIVATE_KEY` set every tool call carries an agent card, and `my_tca("me")` signs the AgentKit challenge when the gate asks for one |
| Buyer agent | `npm run build && npm test` | ✅ tsc clean, **31/31** — the two-act x402 flow, the spend cap, the agent card, and the **reroute** driven through the whole buying loop: the first payment lands on the seller `/tca` suggested and none on the one it named worst |
| Terminal | `npm test && next build` | ✅ **159/159 node tests** (22 suites: buy plan · connection ladder · oracle codec · futures codec · edition · glossary · plain-edition coverage · desk phase · futures book · read result · chain constants · price formatting · indexed tape · verified humans · the agent gate · the agent card · the human proof · the MCP snippet · the reroute decision · the screen verdict · the loop · the throttle) + clean build (11 pages + 29 API routes); `/loop` is the drivable page — six live stations with the agent's own decision function, a Model Armor lab that refuses the demo's injection by name, and a World proof that folds three wallets into one bill; `make verify-loop` asserts every instrument against the deployed alias |
...
| Agent gate + Model Armor | `DEMO_AGENT_API=https://acr-api-1fto.onrender.com make demo-agent` (**2026-09-12**, against production) | ✅ **EVERY ACT HELD** — anonymous → carded → human via `clusterOf` on Arc; a stranger's cluster, last week's id, a wrong audience and a 30-day TTL all refused; a carded tape read carrying an injection **403**, Google naming `pi_and_jailbreak`; `/armor/info` → `backend: gcp`, `screened` climbing from the 0 it read forever while the screen had no call site |
| Human-attributed tape | four demo wallets funded and settled (**2026-09-12**), read back off the subgraph | ✅ **44 of 48** settlements stamped `human: true`; `/rating` `human_share` ≈ 0.22; `/api/humanid` → 2 humans, **2 traded**, 4 wallets. The four missing are named in §8 |
...
- **The carded tier is evadable, by design.** Anyone can mint an agent card — that is what permissionless means — so a per-card limit is a per-key limit, and keys are free. The card buys *attribution*; only the **human** tier buys scarcity, and claiming a human is optional. Said here so nobody reads "three tiers" as three kinds of protection.
- **`scopeHash` is signed and not enforced.** It is in the card, it is in the signature, and the gate checks it against nothing — enforcing it needs a per-route scope map, which is a design decision rather than a wiring one. `/agent/whoami` reports `scope_enforced: false` so it cannot quietly become a field everyone assumes is checked because it is signed.
- **Four human settlements are permanently missing, and the contract is why.** The press wallet ran out of gas mid-run on 2026-09-12; later receipts mirrored before earlier ones, and `ReceiptMirror`'s `backdated for payer` guard — the per-payer monotonic rule that stops a keeper withholding a receipt until the next print moves in its favour — refused the four that were now out of order. That is the guard working, at the cost of four rows. 44 of 48 are on the tape.
- **Human resolutions expire on a clock.** Window 2959 begins **2026-09-17 00:00 UTC**. From that second, every human reads as unresolved — the People chip says "out of date" and `verify_live`'s `humans` pillar names the remedy — until `make resolve-humans ARGS=--commit` runs again. A rotating id is the privacy property; a weekly chore is its price.
...
**One guard in this stack is not Circle's, and it is named rather than folded in.** The payment gate decides who may *buy*; nothing in the Agent Stack inspects what an agent *sends* or what comes back to it. So agent-to-agent traffic — the carded `POST /graph/query` read — passes **Google Cloud Model Armor** in both directions, fail-closed, with `/armor/info` reporting which backend actually answered (`services/index_api/index_api/armor.py`, `agentgate.py`, `scripts/demo_agent.py`). It screens only carded callers: a browser reader behind the Vercel proxy is not agent traffic, and a fail-closed dependency in front of a thousand of those an hour would be a demo that turns itself off.
