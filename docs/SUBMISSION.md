...
| Python suite | `pytest packages services tests` | ✅ **352 passed** — including 9 anvil-gated on-chain tests that CI now genuinely runs (a node is started in the job) rather than silently skipping |
...
| Terminal | `npm test && next build` | ✅ **84/84 node tests** (11 suites: buy plan · connection ladder · oracle codec · futures codec · edition · glossary · plain-edition coverage · desk phase · futures book · read result · chain constants) + clean build (9 pages + 18 API routes) |
