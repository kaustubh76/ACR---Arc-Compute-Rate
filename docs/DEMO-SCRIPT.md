...
| 3 | 1:00–1:45 | `/curve`. The futures desk: series row, **Expiry**, then scroll to **SETTLED ROUNDS · CASH, FINAL** (three ACR-INF rows, `#2 · 0.49533`, `#1 · 0.49533`, `#0 · 0.49270`), then the tape. | “The print has a dependent: an on-chain futures venue that cash-settles against it. *(narrative)* Settlement refuses a print older than two hours — the contract enforces that, so the feed has something that breaks if the feed breaks. And this isn’t a promise about expiry: these rounds already ran their whole life on-chain. Opened, traded for a week, expired, then cash-settled at the oracle print, positions cleared, collateral released. That last one settled today. The desk is public too: anyone can open an account with a PIN and take the other side.” |
