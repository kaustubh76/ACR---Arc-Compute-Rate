...
3. **Demo-Day Metrics** (black frame): the final-slide numbers, and they are measured rather than aspirational — run `make verify-live` for the live set. At time of writing: hourly on-chain prints for 3 indices with attack-cost-per-bp on every one; **4** seller attestations on-chain; **three** live futures books (ACR-INF, ACR-GPU, ACR-DATA) whose maker is a Circle custody wallet, traded hourly by a keeper; **10** real Gateway x402 settlements from **2 distinct payers**; 100% Foundry invariants passing. (The earlier version of this line carried literal `X`/`Y` placeholders and a "5+ sellers" target that was never met — a slide nobody had re-read.)
